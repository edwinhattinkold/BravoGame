...
public:
	...
	void Step(Settings* settings)
	{
		...
		g_debugDraw.DrawString(5, m_textLine, "Press w/a/s/d to control the car");
		m_textLine += 15;
		int angle = m_car->getAngle();
		g_debugDraw.DrawString(5, m_textLine, "angle:%.3f",angle);
		m_textLine += 15;
		int xax = m_car->getPosition().x;
		g_debugDraw.DrawString(5, m_textLine, "x:%.3f", xax);
		m_textLine += 15;
		int yax = m_car->getPosition().y;
		g_debugDraw.DrawString(5, m_textLine, "y:%.3f", yax);
		m_textLine += 15;
